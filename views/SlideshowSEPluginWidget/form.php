<?php if ($data instanceof stdClass) : ?>
	...
	<p>
		<label for="<?php echo $data->widget->get_field_id('title'); ?>"><?php _e('Title', 'slideshow-se'); ?></label>
	...
	<p>
		<label for="<?php echo $data->widget->get_field_id('slideshowId'); ?>"><?php _e('Slideshow', 'slideshow-se'); ?></label>
		<select class="widefat" id="<?php echo $data->widget->get_field_id('slideshowId'); ?>" name="<?php echo $data->widget->get_field_name('slideshowId'); ?>" value="<?php echo (is_numeric($data->instance['slideshowId']))? $data->instance['slideshowId'] : ''; ?>" style="width:100%">
			...
			<?php foreach($data->slideshows as $slideshow): ?>
				<option value="<?php echo $slideshow->ID ?>" <?php selected($data->instance['slideshowId'], $slideshow->ID); ?>><?php echo !empty($slideshow->post_title) ? $slideshow->post_title : __('Untitled slideshow', 'slideshow-se'); ?></option>
			<?php endforeach; ?>
			<?php endif; ?>
		</select>
	</p>
