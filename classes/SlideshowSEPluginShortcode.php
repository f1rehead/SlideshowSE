<?php
/**
 * Class SlideshowSEPluginShortcode provides the shortcode function, which is called
 ...
 */
class SlideshowSEPluginShortcode
{
	...
	static function slideshowDeploy($attributes)
	{
		...
		$settings = SlideshowSEPluginSlideshowSettingsHandler::getSettings($postId);
		...
		else
		{
			...
			$output = SlideshowSEPlugin::prepare($postId);
		}
	...
	/**
	 * Function insertSlideshow uses the prepare method of class SlideshowSEPlugin
	...
	static function insertSlideshow($content)
	{
			...
			count(self::$postIds) > 0)
		{
			...
			{
				$updatedContent = preg_replace("/" . self::$bookmark . "/", SlideshowSEPlugin::prepare($postId), $content, 1);
	...
	static function shortcodeInserter()
	{
		...
		SlideshowSEPluginMain::outputView(__CLASS__ . DIRECTORY_SEPARATOR . 'shortcode-inserter.php', $data);
	}
	...
	static function localizeScript()
	{
		wp_localize_script(
			...
			array(
				...
				'localization' => array('undefinedSlideshow' => __('No slideshow selected.', 'slideshow-se'))
			)
		);
	}
}