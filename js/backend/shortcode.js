slideshow_jquery_image_gallery_backend_script.shortcode = (function () {
	...
	self.activateShortcodeInserter = function () {
		$('.insertSlideshowShortcodeSlideshowInsertButton').click(
			function () {
				var undefinedSlideshowMessage = 'No slideshow selected.',
					shortcode = 'slideshow_deploy',
					slideshowID = parseInt(
						$('#insertSlideshowShortcodeSlideshowSelect').val(),
						10
					),
					extraData =
						window.slideshow_jquery_image_gallery_backend_script_shortcode;
				...
				if (typeof extraData === 'object') {
					...
					if (
						...
						extraData.localization.undefinedSlideshow !==
						undefined &&
						extraData.localization.undefinedSlideshow.length > 0
					) {
						undefinedSlideshowMessage =
							extraData.localization.undefinedSlideshow;
					}
				}
