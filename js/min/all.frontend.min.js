 ...
 */
window.slideshow_jquery_image_gallery_script = window.slideshow_jquery_image_gallery_script || {};
...
(function () {
  ...
  self.Slideshow.prototype.animateTo = function (viewID, direction) {
    ...
    if (this.isVideoPlaying() || viewID < 0 || viewID >= this.$views.length || viewID == this.currentViewID) {
      return;
    }
    ...
    switch (animation) {
      case 'slide':
        ...
        }, this), this.settings.slideSpeed * 1000);
        break;
      case 'slideRight':
        // Prepare next view
        $nextView.css({
      ...
      case 'fade':
        ...
        }, this), this.settings.slideSpeed * 1000 / 2);
        break;
      case 'directFade':
        // Prepare next view
        $nextView.css({
...
(function () {
  ...
  self.Slideshow.prototype.play = function () {
    ...
    this.interval = setInterval($.proxy(function retrieveViewAndAnimateToView(viewID, slideshowInstance) {
      ...
      if (slideshowInstance.isViewLoaded(viewID)) {
        ...
        setTimeout($.proxy(function () {
          retrieveViewAndAnimateToView(viewID, slideshowInstance);
        }, slideshowInstance), 100);
      }
    }, this), this.settings.intervalSpeed * 1000);
  };
  ...
  self.Slideshow.prototype.pause = function (playState) {
    ...
    this.interval = false;
    if (playState !== this.PlayStates.PAUSED && playState !== this.PlayStates.TEMPORARILY_PAUSED) {
  ...
  self.Slideshow.prototype.pauseAllVideos = function () {
    for (var playerID in this.videoPlayers) {
      ...
      if (player != null && typeof player.pauseVideo === 'function' && this.videoPlayers[playerID].state != -1) {
  ...
  self.Slideshow.prototype.getNaturalImageSize = function ($image, callback, data) {
    if ($image.length <= 0 || !($image instanceof $) || typeof $image.attr('src') !== 'string') {
      callback(-1, -1, data);
      return;
    }
  ...
  self.Slideshow.prototype.getNextRandomViewID = function () {
    ...
    if (this.randomAvailableViewIDs === undefined || this.randomAvailableViewIDs.length <= 0) {
...
(function () {
  ...
  self.Slideshow.prototype.recalculateView = function (viewID, forceCalculation) {
    ...
    if ((typeof forceCalculation !== 'boolean' || !forceCalculation) && this.$content.width() == $view.outerWidth(true)) {
      return;
    }
    ...
    $.each($slides, $.proxy(function (slideID, slide) {
      ...
      if ($slide.hasClass('slideshow_slide_text')) {
        ...
        if ($videoElement.length > 0) {
          ...
          var youTubePlayerReadyTimer = setInterval($.proxy(function () {
...
(function () {
  ...
  self.Slideshow = function ($slideshow) {
          ...
          if ($image.length > 0) {
            if ($image.get(0).complete) {
              ...
              this.onImageLoad($image, $.proxy(function (success) {
                ...
                if (this.settings.waitUntilLoaded && viewID === this.currentViewID && this.isViewLoaded(viewID)) {
                  this.start();
                }
              }, this));
            }
    ...
    // Start slideshow
    if (!this.settings.waitUntilLoaded || this.settings.waitUntilLoaded && hasFirstSlideLoaded) {
      this.start();
    }
  };
})();
...
(function () {
  ...
  self.Slideshow.prototype.activateDescriptions = function () {
    ...
    this.$slides.mouseenter($.proxy(function (event) {
      ...
      this.descriptionTimer = setTimeout($.proxy(function () {
        // Reset timer to original value
        this.descriptionTimer = '';

        // Animate pop up
        $description.stop(true, false).animate({
          top: this.$container.outerHeight(true) - $description.outerHeight(true)
    ...
    this.$slides.mouseleave($.proxy(function (event) {
      ...
      $(event.currentTarget).find('.slideshow_description_box').stop(true, false).animate({
        top: this.$container.outerHeight(true)
      }, parseInt(this.settings.descriptionSpeed * 1000, 10));
    }, this));
  };
  ...
  self.Slideshow.prototype.activateNavigationButtons = function () {
    ...
    if (typeof nextText !== 'string' || typeof previousText !== 'string' || nextText.length <= 0 || previousText.length <= 0) {
    ...
    this.$nextButton.html('<span class="assistive-text hide-text">' + nextText + '</span>').attr({
      tabindex: '0',
      title: nextText
    });

    // Add text for screen readers and make button previous keyboard focusable
    this.$previousButton.html('<span class="assistive-text hide-text">' + previousText + '</span>').attr({
      tabindex: '0',
      title: previousText
    });
  ...
  self.Slideshow.prototype.activateControlPanel = function () {
    ...
    this.$container.bind('slideshowPlayStateChange', $.proxy(function (event, playState) {
      ...
      if (typeof playText !== 'string' || typeof pauseText !== 'string' || playText.length <= 0 || pauseText.length <= 0) {
      ...
      if (playState === this.PlayStates.PLAYING) {
        this.$togglePlayButton.html('<span class="assistive-text hide-text">' + pauseText + '</span>').attr({
          class: 'slideshow_pause',
          title: pauseText
        });
      } else if (playState === this.PlayStates.PAUSED) {
        this.$togglePlayButton.html('<span class="assistive-text hide-text">' + playText + '</span>').attr({
  ...
  self.Slideshow.prototype.activatePagination = function () {
    ...
    this.$views.each($.proxy(function (viewID) {
      ...
      // Add list item
      $ul.append('<li ' + 'class="slideshow_transparent ' + currentView + '" ' + 'data-view-id="' + viewID + '" ' + 'role="button" ' + 'title="' + goToText + ' ' + viewNumber + '">' + '<span class="assistive-text hide-text">' + goToText + ' ' + viewNumber + '</span>' + '</li>');
    }, this));

    // On click of a view-bullet go to the corresponding slide
    this.$pagination.find('li').attr('tabindex', '0').click($.proxy(function (event) {
    ...
    this.$container.bind('slideshowAnimationStart', $.proxy(function () {
      ...
      $bullets.each($.proxy(function (key, bullet) {
  ...
  self.Slideshow.prototype.activatePauseOnHover = function () {
    ...
    this.$container.mouseenter($.proxy(function () {
      ...
      this.pauseOnHoverTimer = setTimeout($.proxy(function () {
        this.pause(this.PlayStates.TEMPORARILY_PAUSED);
      }, this), 500);
    }, this));
...
// EXTERNAL MODULE: ./js/frontend/slideshow-namespace.js
var slideshow_namespace = __webpack_require__(485);
// EXTERNAL MODULE: ./js/frontend/slideshow/slideshow.constructor.js
var slideshow_constructor = __webpack_require__(774);
// EXTERNAL MODULE: ./js/frontend/slideshow/slideshow.base.js
var slideshow_base = __webpack_require__(163);
// EXTERNAL MODULE: ./js/frontend/slideshow/slideshow.animation.js
var slideshow_animation = __webpack_require__(713);
// EXTERNAL MODULE: ./js/frontend/slideshow/slideshow.calculation.js
var slideshow_calculation = __webpack_require__(822);
// EXTERNAL MODULE: ./js/frontend/slideshow/slideshow.modules.js
var slideshow_modules = __webpack_require__(76);
;// CONCATENATED MODULE: ./js/frontend/slideshow.js





// EXTERNAL MODULE: ./js/frontend/scriptsLoadedFlag.js
var scriptsLoadedFlag = __webpack_require__(239);
;// CONCATENATED MODULE: ./js/all.frontend.js
/**
...
(function () {
   ...
   */
  self.init = function () {
    if (slideshow_jquery_image_gallery_backend_script_scriptsloadedFlag !== true || self.initialized) {
      return;
    }
  ...
  self.getSlideshowInstance = function (searchKey) {
    if (!isNaN(parseInt(searchKey, 10))) {
      ...
      for (var ID in self.slideshowInstances) {
        ...
        if (slideshowInstance instanceof self.Slideshow && slideshowInstance.$container.get(0) === searchKey.get(0)) {
  ...
  self.activateSlideshow = function ($slideshowElement) {
    if ($slideshowElement.hasClass('slideshow_container') && $slideshowElement.attr('data-slideshow-active') != '1') {
  ...
  self.repairStylesheetURLs = function () {
    ...
    $.each(ajaxStylesheets, function (ajaxStylesheetKey, ajaxStylesheet) {
      ...
      URLData = URL.split('?');
      if (URLData[1] === undefined || URLData[1] === '' || URLData[1].toLowerCase().indexOf('style=') < 0) {
        URLData[1] = 'action=slideshow_jquery_image_gallery_load_stylesheet' + '&style=' + styleName + '&ver=' + Math.round(new Date().getTime() / 1000);
      } else {
        return;
      }
      URL = URLData.join('?');
