(function () {
	...
	self.Slideshow = function ($slideshow) {
		...
		this.$togglePlayButton = this.$controlPanel.find(
			'.slideshow_togglePlay'
		);
		...
		this.settings = window['SlideshowSEPluginSettings_' + this.ID];

		if (!this.settings) {
			try {
				this.settings = JSON.parse(
					this.$container.attr('data-settings')
				);
		...
		var hideViews = $.proxy(function (hideViewsFunction) {
			if (
				...
				setTimeout(
					$.proxy(function () {
						hideViewsFunction(hideViewsFunction);
					}, this),
					500
				);
			}
		...
		if (
			parseFloat(this.settings.intervalSpeed) <
			parseFloat(this.settings.slideSpeed) + 0.1
		) {
